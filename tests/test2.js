...
const { AudioPlayer, createAudioPlayer, createAudioResource, AudioPlayerStatus, AudioInputType } = require('@discordjs/voice');
...
async function searchAndPlay(interaction, query) {
    ...
    // Run yt-dlp to search and download audio in the best quality
    const ytdlpCommand = `${yt_dlp_path} -f bestaudio --extract-audio --audio-quality 0 -o - "ytsearch:${query}"`;

    try {
        // Spawn yt-dlp process and pipe output to ffmpeg
        const ytdlpProcess = exec(ytdlpCommand, {stdio: ['pipe', 'pipe', 'ignore']});

        // Create an ffmpeg process to convert the audio to Opus format and stream it
        const ffmpegProcess = exec('ffmpeg -i pipe:0 -f opus -ar 48000 -ac 2 - |', {stdio: ['pipe', 'pipe', 'ignore']});
        ...
        player.on(AudioPlayerStatus.Idle, () => {
            ...
            connection.destroy(); // Disconnect the bot after audio finishes
        ...
        console.log(`Error: ${e}`);
        const embed = new EmbedBuilder()
            .setTitle("✖️ Error playing audio")
            .setColor("#000000")
